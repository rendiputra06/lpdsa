                            ...
                            <div key={menu.id} className="relative group">
                                ...
                                {menu.children && menu.children.length > 0 && (
                                    <div className="absolute top-full left-0 mt-2 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                        <div className="py-1 bg-white dark:bg-neutral-900 rounded-md shadow-lg border border-slate-200 dark:border-neutral-800">
                                            {menu.children.map((child: any) => (
                                                child.type === 'label' ? (
                                                    <div key={child.id} className="block px-4 py-2 text-sm text-slate-500 font-semibold cursor-default">
                                                        {child.label}
                                                    </div>
                                                ) : (
                                                    <Link
                                                        key={child.id}
                                                        href={child.url}
                                                        className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-univrab-blue dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-univrab-gold"
                                                    >
                                                        {child.label}
                                                    </Link>
                                                )
                                ...
                                {menu.children && menu.children.length > 0 ? (
                                    <>
                                        ...
                                        <div className="pl-4">
                                            {menu.children.map((child: any) => (
                                                child.type === 'label' ? (
                                                    <div key={child.id} className="block rounded-md px-3 py-2 text-sm font-medium text-slate-500 cursor-default">
                                                        {child.label}
                                                    </div>
                                                ) : (
                                                    <Link
                                                        key={child.id}
                                                        href={child.url}
                                                        className="block rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-univrab-blue dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-univrab-gold"
                                                    >
                                                        {child.label}
                                                    </Link>
                                                )
